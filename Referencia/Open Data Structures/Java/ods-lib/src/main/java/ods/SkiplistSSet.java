...
public class SkiplistSSet<T> implements SSet<T> {
     ...
     * @return a node<T> u that maximizes u.x subject to the constraint that u.x < x --- or sentinel if u.x
     * >= x for all node<T>s x
     */
    ...
    /**
     * Simulate repeatedly tossing a coin until it comes up tails. Note, this
     * code will never generate a height greater than 32
     *
    ...
    /**
     * Create a new iterator in which the next value in the iteration is
     * u.next.x TODO: Constant time removal requires the use of a skiplist
     * finger (a stack)
     *
     * @param u
     * @return
