...
public class BinaryTrie<Node extends BinaryTrie.Nöde<Node, T>, T> implements SSet<T> {
    ...
    /**
     * Part of SSet interface, but not really relevant here TODO: We can still
     * implement this
     */
    ...
    /**
     * Find the node in the doubly-linked list that comes before the node that
     * contains (the successor of) x
     *
     * @param x
    ...
    protected Node findPredNode(T x) {
        ...
        if (i == w) {
            ...
            pred = (c == 1) ? u.jump : u.jump.child[0];
        }
        return pred;
    }
