 ...
 * as a single array a. Elements are stored at positions a[0],...,a[size()-1].
...
public class ArrayStack<T> extends AbstractList<T> {
    ...
    /**
     * We override addAll because AbstractList implements it by repeated calls
     * to add(i,x), which can take time O(size()*c.size()). This happens, for
     * example, when i = 0. This version takes time O(size() + c.size()).
     */
    ...
    /**
     * We override this method because AbstractList implements by repeated calls
     * to remove(size()), which takes O(size()) time. This implementation runs
     * in O(1) time.
     */
    public void clear() {
        n = 0;
