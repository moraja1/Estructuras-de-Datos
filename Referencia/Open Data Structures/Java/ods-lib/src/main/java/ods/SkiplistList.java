...
/**
 * Implements the List interface as a skiplist so that all the standard
 * operations take O(log n) time
 *
 * TODO: currently, listIterator() return an iterator that takes O(log n) time
 * per step
 *
 * @author morin
 *
...
public class SkiplistList<T> extends AbstractList<T> {
     ...
     * @return the predecessor of the node at index i or the final node if i
     * exceeds size() - 1.
     */
